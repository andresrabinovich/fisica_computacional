...
int main(int argc, char **argv){
    ...
    float intensidad_cluster_percolante[particiones][repeticiones];
	...
	for(particion = 0; particion < particiones; particion++){
		...
		for(repeticion = 0; repeticion < repeticiones; repeticion++){
            ...
            if(cluster_percolante = verificar_percolacion(alto, ancho, clusters)){
                intensidad_cluster_percolante[particion][repeticion] = (float)masas_de_clusters[cluster_percolante]/(float)masa_total;
            }else{
                intensidad_cluster_percolante[particion][repeticion] = 0;
            }
	...
	sprintf(str, "corridas/ej2/%dx%d.txt", alto, ancho);
    FILE *archivo;
    archivo = fopen(str,"w");
	p = p_inicial;
    for (particion = 0; particion < particiones; particion++) {
		...
		for(repeticion = 0; repeticion < repeticiones; repeticion++){
			...
			//strcat(str, str2);
			strcat(str, "\t");
            sprintf(str2, "%f", intensidad_cluster_percolante[particion][repeticion]);
			strcat(str, str2);
		}
		strcat(str, "\n");
