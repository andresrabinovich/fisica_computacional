...
int main(int argc, char **argv){
	...
	int particion, particiones = 1, repeticiones = 1000;
	if(argc == 3){
	...
	int precision = 10, iteracion;	
	float pc[repeticiones];	
	...
	for(particion = 0; particion < particiones; particion++){
		...
		for(repeticion = 0; repeticion < repeticiones; repeticion++){
			...
			while(iteracion <= precision){
				...
				if(verificar_percolacion(alto, ancho, clusters) == 0){
    ...
    for (repeticion = 0; repeticion < repeticiones; repeticion++) {
    	fprintf(archivo, "%f\n", pc[repeticion]);
		promedio = promedio + pc[repeticion];
    }
	fclose(archivo);
	
	//Mostramos el promedio de las pcriticas
	printf("Pcritica: %f\n", promedio/(float)repeticiones);
